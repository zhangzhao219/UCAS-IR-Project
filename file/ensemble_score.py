...
# log
config_logging("log_ensemble_score")
...
for num in range(2, len(data_list)+1):
    ...
    for i in range(0,len(temp_data)):
        ...
        data_output.to_csv('temp_result_score', index=None, header=None, sep='\t')

        eva = os.popen('trec_eval-9.0.7/trec_eval -m  ndcg_cut ' + 'data/2020/2020qrels-pass.txt' + ' ' + 'temp_result_score').readlines()[1].split(' ')[-1].split('\t')[-1].split('\n')[0]
